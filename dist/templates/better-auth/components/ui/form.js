...
const FormMessage = React.forwardRef((_a, ref) => {
    ...
    return (<AnimatePresence initial={false}>
      {body && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{
                opacity: 1,
                height: "auto",
                transition: { duration: 0.2, opacity: { delay: 0.15 } },
            }} exit={{ opacity: 0, height: 0 }} className="relative w-full overflow-hidden">
          <p ref={ref} id={formMessageId} className={cn("my-1 w-full px-1 text-xs text-red-500", className)} {...props}>
            {body}
          </p>
        </motion.div>)}
    </AnimatePresence>);
