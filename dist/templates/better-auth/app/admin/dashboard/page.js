...
export default function AdminDashboardPage() {
    ...
    const [activeTab, setActiveTab] = useState(appConfig.admin.tabs[0]);
    if (isPending) {
        return <LoadingPage />;
    }
    ...
    return (<div className="min-h-screen">
      ...
      <div className="shadow-sm dark:border-b">
        <Content className="py-6">
          ...
          <div className="mt-4 flex flex-wrap gap-0.5">
            {appConfig.admin.tabs.map((tab) => (<Button key={tab} size={"xs"} className="border-none capitalize duration-0" variant={activeTab === tab ? "default" : "outline"} onClick={() => setActiveTab(tab)}>
                {tab}
              </Button>))}
          </div>
        </Content>
      ...
      <MotionConfig transition={{ duration: 0.1, ease: "easeOut" }}>
        <AnimatePresence mode="wait" initial={false}>
          {activeTab === "dashboard" ? (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key={"dashboard"}>
              <Dashboard />
            </motion.div>) : activeTab === "testimonials" ? (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key={"testimonials"}>
              <Testimonials />
            </motion.div>) : null}
        </AnimatePresence>
      </MotionConfig>
